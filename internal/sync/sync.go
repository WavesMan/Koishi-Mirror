...
func (sm *SyncManager) Pull(ctx context.Context) error {
    // 1. 拉取数据源（带缓存与重试）
    if sm.logger != nil { sm.logger.Info("sync", "开始拉取数据源", map[string]interface{}{"url": sm.config.DataSourceURL}) } else { fmt.Printf("开始拉取数据源: %s\n", sm.config.DataSourceURL) }
    ...
    if sm.logger != nil { sm.logger.Info("sync", "发现数据源包", map[string]interface{}{"total": data.Total}) } else { fmt.Printf("发现 %d 个包\n", data.Total) }
	...
	if err := sm.loadSyncState(ctx); err != nil {
		fmt.Printf("加载同步状态失败，将重新同步所有包: %v\n", err)
	}

	// 5. 确定需要同步的包
	packagesToSync := sm.determinePackagesToSync(data.Packages)
    if sm.logger != nil { sm.logger.Info("sync", "需要同步包", map[string]interface{}{"count": len(packagesToSync)}) } else { fmt.Printf("需要同步 %d 个包\n", len(packagesToSync)) }
	...
	// 7. 保存同步状态
    if err := sm.saveSyncState(ctx); err != nil { if sm.logger != nil { sm.logger.Error("sync", "保存同步状态失败", map[string]interface{}{"error": err.Error()}) } else { fmt.Printf("保存同步状态失败: %v\n", err) } }

	// 8. 处理失败的包
	sm.retryFailedPackages(ctx)

	// 9. 返回错误（如果有）
    if len(syncErrors) > 0 { return fmt.Errorf("同步完成，但有 %d 个包同步失败: %v", len(syncErrors), syncErrors[0]) }
    if sm.logger != nil { sm.logger.Info("sync", "所有包同步完成", nil) } else { fmt.Println("所有包同步完成！") }
	return nil
}
...
func (sm *SyncManager) buildS3Index(ctx context.Context) (map[string]int64, error) {
    ...
    if sm.logger != nil { sm.logger.Debug("s3", "list_objects_done", map[string]interface{}{"count": len(objs), "tgz": len(idx)}) }
    return idx, nil
}
...
func (sm *SyncManager) printInitSummary(ctx context.Context, dsTotal, s3Count int, recon *ReconcileSummary, toSync []models.Package) {
    ...
    fmt.Printf("数据库: 包=%d 版本=%d 状态: success=%d failed=%d pending=%d syncing=%d 最新同步=%s\n",
        pkgs, versions, status["success"], status["failed"], status["pending"], status["syncing"], latest.Format("2006-01-02 15:04:05"))
    ...
    fmt.Printf("对象存储: tgz对象=%d DB缺失S3=%d S3缺失DB=%d 大小不一致=%d\n", s3Count, dbMissing, s3Missing, mismatched)
...
func (sm *SyncManager) syncPackage(ctx context.Context, pkg models.Package) error {
    ...
    if sm.logger != nil { sm.logger.Debug("sync", "download_start", map[string]interface{}{"name": pkg.Name, "version": pkg.Version, "url": pkg.Dist.Tarball}) }
    resp, err := http.Get(pkg.Dist.Tarball)
    if err != nil {
        sm.updatePackageState(pkgKey, "failed", 0)
        if sm.logger != nil { sm.logger.Error("sync", "download_error", map[string]interface{}{"name": pkg.Name, "version": pkg.Version, "error": err.Error()}) }
    ...
    if err != nil {
        sm.updatePackageState(pkgKey, "failed", 0)
        if sm.logger != nil { sm.logger.Error("sync", "write_temp_error", map[string]interface{}{"name": pkg.Name, "version": pkg.Version, "error": err.Error()}) }
    ...
    if err := sm.s3Client.UploadFile(ctx, s3Key, tempFile, "application/gzip", fileSize); err != nil {
        sm.updatePackageState(pkgKey, "failed", 0)
        if sm.logger != nil { sm.logger.Error("s3", "upload_error", map[string]interface{}{"key": s3Key, "error": err.Error()}) }
    ...
    if err := sm.savePackageMetadata(ctx, pkg); err != nil { if sm.logger != nil { sm.logger.Error("sync", "save_meta_error", map[string]interface{}{"name": pkg.Name, "version": pkg.Version, "error": err.Error()}) } else { fmt.Printf("保存包元数据失败 %s@%s: %v\n", pkg.Name, pkg.Version, err) } }

    // 更新包状态为同步成功
    sm.updatePackageState(pkgKey, "success", fileSize)
    if sm.logger != nil { sm.logger.Info("sync", "同步成功", map[string]interface{}{"name": pkg.Name, "version": pkg.Version, "key": s3Key, "size": fileSize}) } else { fmt.Printf("同步成功: %s@%s -> S3:%s (大小: %d bytes)\n", pkg.Name, pkg.Version, s3Key, fileSize) }
...
func (sm *SyncManager) Reconcile(ctx context.Context) (*ReconcileSummary, error) {
    ...
    for _, r := range rows {
        ...
        if err != nil {
            if sm.logger != nil { sm.logger.Debug("s3", "head_missing", map[string]interface{}{"name": name, "version": version}) }
        ...
        if info.Size != nil { actual = *info.Size }
        if actual != size {
            if sm.logger != nil { sm.logger.Debug("s3", "size_mismatch", map[string]interface{}{"name": name, "version": version, "db": size, "s3": actual}) }

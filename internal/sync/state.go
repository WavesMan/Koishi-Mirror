...
func (sm *SyncManager) updatePackageState(pkgKey, status string, size int64) {
    ...
    if !exists { state = models.PackageState{} }

    state.SyncStatus = status
    state.SyncTime = time.Now()
    if status == "failed" { state.RetryCount++ }
    if status == "success" { state.Size = size }
...
func (sm *SyncManager) updateSyncStats() {
    ...
    for _, state := range sm.syncState.PackageStates {
        if state.SyncStatus == "success" { synced++ } else if state.SyncStatus == "failed" { failed++ }
    }
...
func (sm *SyncManager) retryFailedPackages(ctx context.Context) {
    ...
    if len(failedPackages) == 0 { return }
    ...
    for _, pkg := range failedPackages {
        ...
        go func(p models.Package) {
            ...
            defer func() { <-sem }()
            fmt.Printf("重试同步: %s@%s\n", p.Name, p.Version)
            if err := sm.fetchPackageInfo(ctx, &p); err != nil { fmt.Printf("获取包信息失败 %s@%s: %v\n", p.Name, p.Version, err); return }
            if err := sm.syncPackage(ctx, p); err != nil { fmt.Printf("重试失败 %s@%s: %v\n", p.Name, p.Version, err) } else { fmt.Printf("重试成功 %s@%s\n", p.Name, p.Version) }
        }(pkg)
    }
    wg.Wait()
    if err := sm.saveSyncState(ctx); err != nil { fmt.Printf("保存同步状态失败: %v\n", err) }
}
...
func (sm *SyncManager) fetchPackageInfo(ctx context.Context, pkg *models.Package) error {
    ...
    for _, p := range d.Packages {
        if p.Name == pkg.Name && p.Version == pkg.Version { *pkg = p; return nil }
    }
...
func (sm *SyncManager) savePackageMetadata(ctx context.Context, pkg models.Package) error {
    ...
    if err != nil { return fmt.Errorf("序列化元数据失败: %v", err) }
...
func (sm *SyncManager) loadSyncState(ctx context.Context) error {
    ...
    if err := json.NewDecoder(r).Decode(&st); err != nil { return err }
    sm.stateMutex.Lock()
    sm.syncState = &st
    sm.stateMutex.Unlock()
    return nil
...
func (sm *SyncManager) saveSyncState(ctx context.Context) error {
    if sm.store != nil { return sm.store.SaveSyncState(ctx, sm.syncState) }
    key := sm.s3Client.GetSyncStateKey()

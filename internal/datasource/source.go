...
func (s *Source) ClearCache(ctx context.Context) {
    if s.cache != nil { _ = s.cache.Del(ctx, "data_source:index") }
    if s.logger != nil { s.logger.Info("cache", "data_source_cleared", nil) }
}
...
func (s *Source) Get(ctx context.Context) (models.DataSource, error) {
    ...
    if s.cache != nil {
        if raw, err := s.cache.Get(ctx, "data_source:index"); err == nil && raw != "" {
            ...
            if s.logger != nil { s.logger.Debug("datasource", "cache_hit", map[string]interface{}{"packages": len(ds.Packages), "total": ds.Total}) }
            if len(ds.Packages) > 0 { return ds, nil }
        }
    }
    ...
    for i := 0; i < s.cfg.DataSourceMaxRetries; i++ {
        ...
        if s.logger != nil { s.logger.Debug("datasource", "fetch_start", map[string]interface{}{"url": s.cfg.DataSourceURL}) }
        resp, err := s.client.Do(req)
        if err != nil {
            lastErr = err
            if s.logger != nil { s.logger.Error("datasource", "fetch_error", map[string]interface{}{"error": err.Error()}) }
        } else {
            defer resp.Body.Close()
            b, _ := io.ReadAll(resp.Body)
            if s.logger != nil { s.logger.Debug("datasource", "fetch_done", map[string]interface{}{"status": resp.StatusCode, "bytes": len(b)}) }
            // first try strict decode
            if err := json.Unmarshal(b, &ds); err == nil && (len(ds.Packages) > 0) {
                if ds.Total == 0 { ds.Total = len(ds.Packages) }
                if bb, err := json.Marshal(ds); err == nil && s.cache != nil { _ = s.cache.Set(ctx, "data_source:index", string(bb)) }
                if s.logger != nil { s.logger.Debug("datasource", "parse_strict", map[string]interface{}{"packages": len(ds.Packages)}) }
                return ds, nil
            }
            // fallback normalize
            nds := normalizeDataSource(b)
            if len(nds.Packages) > 0 {
                if nds.Total == 0 { nds.Total = len(nds.Packages) }
                if bb, err := json.Marshal(nds); err == nil && s.cache != nil { _ = s.cache.Set(ctx, "data_source:index", string(bb)) }
                if s.logger != nil { s.logger.Debug("datasource", "parse_normalized", map[string]interface{}{"packages": len(nds.Packages)}) }
                return nds, nil
            }
            lastErr = err
            if s.logger != nil { s.logger.Warn("datasource", "parse_empty", map[string]interface{}{"total": ds.Total}) }
        }
    ...
    if s.cache != nil {
        if raw, err := s.cache.Get(ctx, "data_source:index"); err == nil && raw != "" {
            _ = json.Unmarshal([]byte(raw), &ds)
            if s.logger != nil { s.logger.Debug("datasource", "cache_fallback", map[string]interface{}{"packages": len(ds.Packages), "total": ds.Total}) }
            return ds, nil
        }
    }

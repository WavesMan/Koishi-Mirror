...
// NOTE: 状态统计接口集中此处，统一数据来源（DB 优先、内存回退），保证接口稳定性。
func (h *Handler) GetMirrorStatus(c *gin.Context) {
    ...
    if h.store != nil {
        if m, err := h.store.StatusCounts(c.Request.Context()); err == nil {
            if v, ok := m["success"]; ok { synced = v }
            if v, ok := m["failed"]; ok { failed = v }
        }
        if sz, err := h.store.TotalSuccessSize(c.Request.Context()); err == nil { storageSize = sz }
        if t, err := h.store.LatestSyncTime(c.Request.Context()); err == nil { lastSync = t }
    } else {
        for _, state := range syncState.PackageStates {
            if state.SyncStatus == "success" { synced++; storageSize += state.Size } else if state.SyncStatus == "failed" { failed++ }
        }
    }

    status := models.MirrorStatus{LastSyncTime: lastSync, TotalPackages: total, SyncedPackages: synced, FailedPackages: failed, StorageSize: storageSize, S3Bucket: h.config.TENCENT_COSBucket, DataSourceURL: h.config.DataSourceURL}
    breakdown := map[string]int{"success": synced, "failed": failed}
    if h.store != nil {
        if m, err := h.store.StatusCounts(c.Request.Context()); err == nil { breakdown = m }
    } else {
        pending := 0
        syncing := 0
        for _, state := range syncState.PackageStates {
            switch state.SyncStatus { case "pending": pending++; case "syncing": syncing++ }
        }
...
func (h *Handler) GetMirrorCOSStatus(c *gin.Context) {
    ...
    total := syncState.TotalPackages
    if h.ds != nil {
        if d, err := h.ds.Get(c.Request.Context()); err == nil { if d.Total > 0 { total = d.Total } }
    }
...
func (h *Handler) GetStorageStats(c *gin.Context) {
    if h.store != nil {
        ...
        if err == nil && len(versions) > 0 {
            ...
            for _, v := range versions { stats.TotalSize += v.Size; packageVersions[v.Name] = append(packageVersions[v.Name], v.Size) }
            ...
            for name, sizes := range packageVersions {
                ...
                for _, size := range sizes { stat.TotalSize += size; if size > stat.MaxSize { stat.MaxSize = size }; if size < stat.MinSize { stat.MinSize = size } }

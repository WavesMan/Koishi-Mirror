...
// NOTE: 领域拆分：包列表与详情相关接口集中此处，保证职责清晰、便于维护与扩展。
func (h *Handler) GetPackages(c *gin.Context) {
    page := 1
    pageSize := 50
    if v := c.DefaultQuery("page", "1"); v != "" { if n, err := strconv.Atoi(v); err == nil { page = n } }
    if v := c.DefaultQuery("pageSize", "50"); v != "" { if n, err := strconv.Atoi(v); err == nil { pageSize = n } }
    ...
    if search != "" {
        ...
        for _, pkg := range packages {
            if strings.Contains(pkg.Name, search) { filtered = append(filtered, pkg) }
        }
        packages = filtered
    }
    ...
    if start >= len(packages) { start = 0; end = 0 }
    if end > len(packages) { end = len(packages) }

    var pagedPackages []models.Package
    if end > start { pagedPackages = packages[start:end] }
    ...
    if h.store != nil {
            ...
            if versions, err := h.store.GetPackageVersions(c.Request.Context(), pagedPackages[i].Name); err == nil {
                for _, v := range versions {
                    if v.Version == pagedPackages[i].Version {
                        ...
                        if v.Dist.Size > 0 { pagedPackages[i].Dist.Size = v.Dist.Size }
                        break
                    }
                }
            }
            if pagedPackages[i].SyncStatus == "" { pagedPackages[i].SyncStatus = "pending" }
        }
    } else {
        ...
        for i := range pagedPackages {
            ...
            if state, exists := syncState.PackageStates[pkgKey]; exists {
                ...
                if state.Size > 0 { pagedPackages[i].Dist.Size = state.Size }
            } else {
                ...
                if versions, err := h.store.GetPackageVersions(c.Request.Context(), pkg.Name); err == nil {
                    for _, v := range versions {
                        if v.Version == pkg.Version {
                            ...
                            syncTime = v.SyncTime
                            if v.Dist.Size > 0 { size = v.Dist.Size }
                            break
                        }
                    }
                }

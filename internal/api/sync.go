...
// NOTE: 同步与后台任务接口集中管理，统一触发入口与回退策略。
func (h *Handler) LogTest(c *gin.Context) {
    if h.store == nil { c.JSON(http.StatusInternalServerError, gin.H{"error": "store not configured"}); return }
...
func (h *Handler) BootSync(c *gin.Context) {
    go func() { _ = h.syncManager.BootSync(context.Background()) }()
    c.JSON(http.StatusOK, gin.H{"message": "已触发初始化同步"})
}

func (h *Handler) TriggerSync(c *gin.Context) {
    go func() { if err := h.syncManager.Pull(context.Background()); err != nil { fmt.Printf("手动触发同步失败: %v\n", err) } }()
...
func (h *Handler) AdvisoriesBulk(c *gin.Context) { c.JSON(http.StatusOK, gin.H{"advisories": []interface{}{}, "objects": []interface{}{}}) }

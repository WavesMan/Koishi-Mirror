...
func New(cfg *config.Config) (*Client, error) {
	...
	s3Client := s3.NewFromConfig(awsCfg, func(o *s3.Options) {
		...
		// 对于自定义域名（如 ks-oss.waveyo.cn），明确禁用 PathStyle
		// 因为它 CNAME 到了 Virtual-Hosted-Style 的 COS 地址
		if strings.Contains(endpointURL.Host, "waveyo.cn") {
			o.UsePathStyle = false
		}
...
func (c *Client) UploadFile(
	...
	if c.logger != nil {
		...
		if out.VersionId != nil {
			f["version_id"] = aws.ToString(out.VersionId)
		}
...
func (c *Client) GetFileInfo(ctx context.Context, key string) (*types.Object, error) {
	...
	if c.logger != nil {
		...
		if resp.LastModified != nil {
			f["last_modified"] = aws.ToTime(resp.LastModified)
		}
